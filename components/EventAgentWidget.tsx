...
export default function EventAgentWidget() {
  ...
  // 🔥 usePaymentPlans removed — this widget always creates a Draft, which
  // is free. Credits are only consumed when an event is switched to Active
  // (the status toggle on /events).
  ...
  const handleSend = async () => {
    ...
    // 🔥 REMOVED: `if (!canCreateEvent())` block — this was the "You have
    // reached your event limit" toast blocking ALL AI-created events,
    // including drafts. Draft creation must never be gated by plan credits.
    ...
    try {
      ...
      if (data.type === 'ready') {
        ...
        const eventId = await createEvent({
          ...
          // 🔥 Always saved as Draft — free, no plan credit used.
          status: 'draft',
        });

        if (!eventId) throw new Error('Failed to create event');

        // 🔥 REMOVED: incrementEventCount() + refreshPlan() — draft
        // creation must never touch plan credits.
